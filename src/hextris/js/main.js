...
function showHelp() {
	...
	$("#inst_main_body").html("<div id='instructions_head'>Como xogar?</div><p>O obxectivo de Hextris é evitar que os bloques saian do interior do hexágono gris exterior.</p><p>" + (settings.platform != 'mobile' ? 'Preme as teclas de frecha dereita e esquerda' : 'Toca os lados esquerdo e dereito da pantalla') + " para rotar o hexágono." + (settings.platform != 'mobile' ? ' Preme a tecla de frecha abaixo para acelerar a caída do bloque': '') + " </p><p>Limpa bloques e obtén puntos facendo que 3 ou máis bloques da mesma cor se toquen.</p><p>O tempo que queda antes de que a túa racha de combos desapareza indícase por <span style='color:#f1c40f;'>as</span> <span style='color:#e74c3c'>liñas</span> <span style='color:#3498db'>de</span> <span style='color:#2ecc71'>cor</span> no hexágono exterior</p><hr>");
	if (gameState == 1) {
		pause();
	}
