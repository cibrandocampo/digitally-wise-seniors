...
var Game = new (function() {
  var self = this,
      ...
      currentPuzzle = null,
      checkTOH = 0,
      ojoos = ['Maravilloso', 'Espectacular', 'Asombroso', 'Sobresainte', 'Notable', 'Impresionante', 'Xenial', 'Ben feito', 'Fabuloso', 'Inxenioso', 'Deslumbrante', 'Eres a crema', 'Excelente', 'Super', 'Brillante', 'Excepcional'],
      remainingOjoos = [],
      endGameTOH1,
      endGameTOH2,
      endGameTOH3,
  ...
  function showSizes() {
    onHomeScreen = false;
    showGame();
    $('#boardsize').html('<span>Grella</span>');
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('Preme para desfacer...');
      else
        grid.hint.show('Non hai nada que desfacer!');
      return;
    }
    ...
    tile.mark();
    var s = 'O azulexo invertiuse a ';
    if (value == 1) s += 'vermello.';
    if (value == 2) s += 'azul.';
    if (value == 0) s += 'baleiro.'
    grid.hint.show(s);
    undone = true;
    clearTimeout(checkTOH);
    checkTOH = setTimeout(function(){checkForLevelComplete();}, 700);
